...
namespace UnityStandardAssets.Cameras
{
    ...
    {
        public float clipMoveTime = 0f;              // time taken to move when avoiding cliping (low value = fast, which it should be)
        public float returnTime = 0.04f;                 // time taken to move back towards desired position, when not clipping (typically should be a higher value than clipMoveTime)
        public float sphereCastRadius = .1f;           // the radius of the sphere used to test for object between camera and target
